...
% model and threshold-grid search logic as the original fixed-scale LFP
% analysis.
...
if isfile(cfg.cache_mat)
    ...
    if has_required_cache
        ...
        if cache_size_ok && cache_grid_ok
            use_cache = true;
        end
    end
end
...
function sampled_noise = local_build_noise_by_k( ...
    ...
    base_u, base_z, base_pick)
% Build residual noise samples for a given target-domain label size k.

k_emp = k_emp(:);
k_vals = k_vals(:);

idx_curve = find(k_vals == k_current, 1);

if isempty(idx_curve)
    error('k_current = %d was not found in k_vals.', k_current);
end

param_noise = build_parametric_noise( ...
    idx_curve, ...
...
if ~isempty(idx_emp)
    ...
    e_pool = err_cell{idx_emp};
    e_pool = e_pool(:);
    e_pool = e_pool(isfinite(e_pool));

    if isempty(e_pool)
        sampled_noise = param_noise;
        return;
    end

    pick_idx = 1 + floor(base_pick .* numel(e_pool));
    pick_idx(pick_idx < 1) = 1;
    pick_idx(pick_idx > numel(e_pool)) = numel(e_pool);

    sampled_noise = e_pool(pick_idx);

    return;
end

if k_current > k_anchor && k_current <= k_bridge_end

    idx_anchor = find(k_emp == k_anchor, 1);

    if ~isempty(idx_anchor)

        e_anchor = err_cell{idx_anchor};
        e_anchor = e_anchor(:);
        e_anchor = e_anchor(isfinite(e_anchor));

        if ~isempty(e_anchor)

            pick_idx = 1 + floor(base_pick .* numel(e_anchor));
            pick_idx(pick_idx < 1) = 1;
            pick_idx(pick_idx > numel(e_anchor)) = numel(e_anchor);

            empirical_anchor_noise = e_anchor(pick_idx);

            alpha = (k_current - k_anchor) / max(k_bridge_end - k_anchor, eps);
            alpha = min(max(alpha, 0), 1);

            sampled_noise = (1 - alpha) .* empirical_anchor_noise + alpha .* param_noise;
...
function param_noise = build_parametric_noise( ...
    idx_curve, ...
    w_curve, muM_curve, sigM_curve, muR_curve, sigR_curve, ...
    base_u, base_z)
% Build two-mode parametric residual noise samples for a curve index.

w = w_curve(idx_curve);
muM = muM_curve(idx_curve);
sigM = sigM_curve(idx_curve);
muR = muR_curve(idx_curve);
sigR = sigR_curve(idx_curve);

use_right = base_u < w;

param_noise = muM + sigM .* base_z;
param_noise(use_right) = muR + sigR .* base_z(use_right);

param_noise = min(max(param_noise, -0.10), 0.10);
end
