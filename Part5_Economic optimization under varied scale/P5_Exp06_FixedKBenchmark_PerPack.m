...
cfg.lfp_fig = fullfile(cfg.extended_fig_dir, ...
    'ExD06a_LFP_FixedKBenchmark_PerPack.fig');

cfg.lfp_png = fullfile(cfg.extended_fig_dir, ...
    'ExD06a_LFP_FixedKBenchmark_PerPack.png');

cfg.nmc_fig = fullfile(cfg.extended_fig_dir, ...
    'ExD06b_NMC_FixedKBenchmark_PerPack.fig');

cfg.nmc_png = fullfile(cfg.extended_fig_dir, ...
    'ExD06b_NMC_FixedKBenchmark_PerPack.png');
...
fig_lfp = plotPerPackBenchmark( ...
    LFP, ...
    'Extended Data Fig. 6a - LFP fixed-k policy regret per pack', ...
    marker_list, ...
    color_list, ...
    line_width, ...
    marker_size, ...
...
fig_nmc = plotPerPackBenchmark( ...
    NMC, ...
    'Extended Data Fig. 6b - NMC fixed-k policy regret per pack', ...
    marker_list, ...
    color_list, ...
    line_width, ...
    marker_size, ...
...
function out = processFixedKPerPackBenchmark(chemistry, primary_file, legacy_file, benchmark_k)
    ...
    [opt_total_cost, opt_cost_source] = readOptionalVectorByCandidates(R, ...
        getOptCostCandidates(chemistry));

    if isempty(opt_total_cost)
        opt_total_cost = opt_total_cost_derived;
        opt_cost_source = 'derived from Total_Cost_NK';
    else
        opt_total_cost = double(opt_total_cost(:));
    end

    if numel(opt_total_cost) ~= n_scale
        error('%s optimized total cost length is inconsistent with scale_ton_list.', chemistry);
    end

    [opt_k, opt_k_source] = readOptionalVectorByCandidates(R, ...
        getOptKCandidates(chemistry));
    ...
    if numel(opt_k) ~= n_scale
        error('%s optimized k length is inconsistent with scale_ton_list.', chemistry);
    end
    ...
    fprintf('Max |used optimized cost - matrix minimum| = %.6g USD\n', max_cost_diff);

    if max_cost_diff > 1e-5 * cost_ref
        warning(['%s optimized costs differ from the minimum of Total_Cost_NK. ' ...
            'Please check whether the result file is internally consistent.'], chemistry);
    end
    ...
    if any(additional_cost(:) < -negative_tol)
        warning(['%s has fixed-k benchmark costs lower than the optimized policy cost. ' ...
            'Please check Total_Cost_NK and optimized cost variables.'], chemistry);
    end
